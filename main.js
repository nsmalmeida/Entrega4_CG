...
// 2. CHECKAGEM POTÊNCIA DE 2 (Power of 2)
...
function loadTexture(gl, url) {
  ...
  image.onload = () => {
    ...
    if (isPowerOf2(image.width) && isPowerOf2(image.height)) {
       ...
       // Não é potência de 2: desativa mips e define clampping
...
// 1. REFATORAÇÃO DOS DADOS DO CUBO (Modularizado)

function setCubeVertices(v) {
  // Frente, Trás, Topo, Fundo, Direita, Esquerda
...
function createCubeData() {
  // Chama as funções auxiliares passando 0.5 para manter o tamanho original
...
function main() {
  ...
  // 3. CONFIGURAÇÕES ESTÁTICAS MOVIDAS PARA FORA DO LOOP
  ...
  function drawScene() {
    ...
    // Apenas limpa o buffer a cada quadro
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);

    // Como gl.useProgram já foi chamado fora, não precisamos chamar aqui
    // A menos que você trocasse de programa shaders durante a execução
